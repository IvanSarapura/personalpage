...
- Chromium ejecuta la matriz completa; Firefox y WebKit cubren axe, interacción, medios y responsive. Los snapshots canónicos se generan sólo en Ubuntu 24.04/Chromium, con fuentes/imágenes cargadas y estabilización visual acotada a las capturas. La estrategia combina diez rutas en viewports de 390×844 y 1440×900, estados interactivos críticos y regiones focales below-fold de home, footer, about, listado de blog y artículo. Así se conserva contexto de página y detalle útil sin depender de capturas `fullPage` gigantes y frágiles;
- `npm run test:e2e:update` crea primero el build de producción, inicia su propio `next start` y actualiza en serie los 56 baselines de Chromium. Sólo debe ejecutarse cuando el cambio visual sea intencional y todas las imágenes resultantes puedan revisarse antes de integrarlas.
